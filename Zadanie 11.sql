...
-- CREATE PROCEDURE wypisz_bilety()
-- BEGIN
--     SELECT * FROM bilety;
-- END; 
...
-- CREATE PROCEDURE najw_cargo()
-- BEGIN
--     SELECT max(cargo) FROM rejsy;
-- END;

-- call najw_cargo();


-- CREATE PROCEDURE zyski ()
--     SELECT SUM(cena) FROM bilety;

-- CALL zyski();


CREATE PROCEDURE wypisz_wszystko()
BEGIN
...
CALL wypisz_wszystko();
CALL samoloty_obslugujace_wiecej_niz_jedna_trase();
CALL add_passengers();

CREATE VIEW first_class AS SELECT * FROM bilety WHERE klasa="pierwsza";
SELECT * FROM first_class;


CREATE VIEW wszystkie_samoloty AS SELECT a.producent, a.model, b.miejsce_wylotu, b.kierunek_docelowy 
FROM samoloty a, rejsy b WHERE a.id=b.id;
SELECT * FROM wszystkie_samoloty;
